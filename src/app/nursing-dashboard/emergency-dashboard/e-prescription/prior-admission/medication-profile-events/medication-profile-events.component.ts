...
export class MedicationProfileEventsComponent {
  ...
  Createeventdata() {
    ...
    if (filterSelectedData && filterSelectedData.length) {
      this.eventsprofileForm.get('MedicationHold').patchValue({
        ...
        // Stoid: filterSelectedData[0].Stoid,
  ...
  SuspendAction() {
    ...
    if (new Date() <= filterSelectedData[0].Pbdad) {
      if (filterSelectedData && filterSelectedData.length || newtime <= filterSelectedData[0].Pbtad) {
        ...
        this.eventActionSet(PayloadData, "Event has been Suspended", "Suspend")
      }
    } else {
  ...
  endEventAction() {
    ...
    if (filterSelectedData && filterSelectedData.length) {
      ...
      const PayloadData = {
        ...
        Datefrom: `${formatDate(this.eventsprofileForm.get('MedicationEnd').value.Datefrom, "YYYY-MM-DD")}T00:00:00`,
        Timefrom: Timefrom,
        Action: "1",
      }
      this.eventActionSet(PayloadData, "Event has been Ended")
    }
  }
  ...
  editEventAction() {
    ...
    if (new Date() <= filterSelectedData[0].Pbdad) {
      if (filterSelectedData && filterSelectedData.length) {
        ...
        this.eventActionSet(PayloadData, "Order has been Edited")
      }
    } else {
  ...
  deleteeventAction() {
    ...
    if (new Date() <= filterSelectedData[0].Pbdad) {
      if (filterSelectedData && filterSelectedData.length) {
        ...
        this.eventActionSet(PayloadData, "Event has been deleted")
      }
    } else {
  ...
  ResumeeventAction() {
    ...
    if (filterSelectedData && filterSelectedData.length) {
      ...
      this.eventActionSet(PayloadData, "Event has been Resumed", "Resume")
    }
  }

  eventActionSet(data: any, message: string, eventType?: string) {
