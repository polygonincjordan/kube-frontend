...
export class EMarOrderHistoryComponent {
  ...
  nextDate() {
    const filterData = this.allEventData.filter(d => d.ParsedDate > this.currentDate);
    this.currentDate = filterData && filterData.length ? filterData.reduce(function (a, b) { return a.ParsedDate < b.ParsedDate ? a : b; }).ParsedDate : this.currentDate;
    this.setCurrentDateData();
  }
