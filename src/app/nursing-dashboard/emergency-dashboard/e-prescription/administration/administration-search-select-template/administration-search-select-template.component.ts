...
export class AdministrationSearchSelectTemplateComponent implements OnInit {
  ...
  broadcastEvent(event: any) {
    if (event.Tmptype === "1") {
      this.ePrescriptionService.loadData(`e-prescription/orderTemplateMedication?EINRI=${this.ePrescriptionService.parameters.einri}&FALNR=${this.ePrescriptionService.parameters.falnr}&PRSCRID=${event.Prscrid}&Ordtype=${'1'}`, false, false, false, false).subscribe((resp: any) => {
