...
export class HeaderComponent implements OnInit {
  ...
  //patient search
  patientSearch(template, mrnOverride?: string) {
    // The merged-patient redirect searches the MRN SAP returned, which is
    // zero-padded, while the form field shows it unpadded for readability.
    const patnr = mrnOverride ?? this.f.mrn.value;
    let jsonObj;
    if (this.f.dob.value) {
