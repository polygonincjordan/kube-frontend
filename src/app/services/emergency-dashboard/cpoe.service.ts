...
export class CpoeService {
  ...
  loadeOrderData() {
    this.checkedlabOrdersCount = 0;
    this.checkedradOrdersSearchCount = 0;
    this.checkedprocedureSearchCount = 0;
    this.loadParameters();
    if (
  ...
  /** Publishes a configuration to the screen and loads the data its tabs need. */
  private applyConfiguration(config: any) {
    this.configurationoption = config;
    this.configurationoptionBackup = JSON.parse(JSON.stringify(config));
  ...
  countdata(listType: string) {
    const checkbox = event.target as HTMLInputElement;
    const isChecked = checkbox.checked;
    if (isChecked && this.labOrdersSearchList && this.radOrdersSearchList && this.procedureSearchList) {
      if (listType === 'LAB' && typeof listType !== 'undefined') {
        this.checkedlabOrdersCount++;
      }
      if (listType === 'RAD' && typeof listType !== 'undefined') {
        this.checkedradOrdersSearchCount++;
      }
      if (listType === 'PROC' && typeof listType !== 'undefined') {
        this.checkedprocedureSearchCount++;
      }
    } else {
      if (listType === 'LAB' && typeof listType !== 'undefined') {
        this.checkedlabOrdersCount--;
  ...
  onAddOrderCheckbox(event: any, element: any, listType: string) {
    const checkbox = event.target as HTMLInputElement;
    const isChecked = checkbox.checked;
    this.countdata(listType)
  ...
  generateEorder() {
    if (this.eOrdersMaster && this.eOrdersMaster.length > 0) {
      if (
        this.eOrdersMaster[0].dataType === 'searchListData' ||
        this.eOrdersMaster[0].dataType === 'CopyHistoryData'
      ) {
        this.switchVal.name = 'eorders';
      } else if (this.eOrdersMaster[0].dataType === 'EditHistoryData') {
        this.switchVal.name = 'history-data';
      } else {
        this.switchVal.name = 'history';
      }
      // if(this.eOrdersMaster[0].dataType !== "EditHistoryData"){
      //   this.switchVal.name = "history-data"
      // }
    }
    if (this.eOrdersMaster && this.eOrdersMaster.length === 0) {
    ...
    if (this.feeServiceOrderData && this.feeServiceOrderData.length > 0) {
      ...
      if (
        this.feeServiceOrderData[0].dataType === 'FeesDataType' ||
        this.feeServiceOrderData[0].dataType === 'CopyFeeHistoryData' ||
        this.feeServiceOrderData[0].dataType === 'EditFeeHistoryData'
      ) {
  ...
  validateEPresc(element: any) {
    ...
    if (element.Templatemode) {
      element.groupItem.forEach((item: any) => {
        ...
        if (item.dosageCountDesc == null || item.dosageCountDesc === "") {
  ...
  onDeleteOrder(deletedata: any) {
    ...
    this.generateEorder();
    this.countdata(deletedata.type)
  }
  ...
  resetView() {
    let that = this;
    that.eOrdersMaster = null;
    that.eOrders = null;
    if (this.labOrdersSearchList.length > 0) {
      this.labOrdersSearchList.forEach((obj: any) => {
        obj.isSelected = false;
      });
    }
    if (this.radOrdersSearchList.length > 0) {
      this.radOrdersSearchList.forEach((obj: any) => {
        obj.isSelected = false;
      });
    }
    if (this.procedureSearchList.length > 0) {
      this.procedureSearchList.forEach((obj: any) => {
        obj.isSelected = false;
      });
    }
    this.switchVal.name = 'history';
    this.labOrdersSearchList = [...this.labOrdersSearchList];
    this.radOrdersSearchList = [...this.radOrdersSearchList];
    this.procedureSearchList = [...this.procedureSearchList];
    if (this.navigationTab == 'Clinical') {
      this.loadClinicalOrder();
    } else if (this.navigationTab == 'Medications') {
      this.loadMedicalOrder();
    } else if (this.navigationTab == 'Fees') {
      this.loadFeesOrder();
    }
    this.feeServiceData = '';
  }
  ...
  loadHistory() {
    ...
    let filter = {
      ...
      falnr: this.constants.falnr,
      lfdnr: this.constants.lfdnr,
      Patnr: this.constants.patnr,
    };
    let expandEntities = ['TOLABSET', 'TORADSET', 'TOPROCSET', 'TOMEDICSET'];
    this.dataService
      .loadData(
        'OrderSet',
        null,
      ...
      .subscribe(
        (resp: any) => {
          ...
          if (success.d.results.length > 0) {
            success.d.results.forEach((obj: any) => {
              obj.orderId = Number(obj.Eorderid2).toString();
              const erDate = obj.Erdat
                ? +obj.Erdat.replace(/[^0-9]/g, '')
                : null;
              obj.creationDate = erDate
                ? this.datePipe.transform(
                  erDate.toString().replace(/\//g, ''),
                  'dd.MM.yyyy'
                )
                : '';
              obj.creationTime = this.uiHelper.formatTime(obj.Ertim);

              if (obj.TOLABSET?.results && obj.TOLABSET?.results.length) {
                obj.TOLABSET.results.forEach((labObj: any) => {
                  const wbgtmpDate = labObj.Wbgtmp
                    ? +labObj.Wbgtmp.replace(/[^0-9]/g, '')
                    : null;
                  labObj.einri = obj.einri;
                  labObj.falnr = obj.falnr;
                  labObj.lfdnr = obj.lfdnr;
                  labObj.Patnr = obj.Patnr;
                  labObj.catalogkey = labObj.Tarif;
                  labObj.clinOrdTypId = labObj.Cordtypid;
                  labObj.service = labObj.Talst;
                  labObj.serviceDescr = labObj.Servtxt;
                  labObj.id = labObj.service;
                  labObj.text = labObj.Servtxt;
                  labObj.isFavourite = labObj.Favourite;
                  labObj.icon = labObj.isFavourite ? 'star' : 'star_border';
                  labObj.isSelected = false;
                  labObj.type = 'LAB';
                  labObj.date = wbgtmpDate
                    ? this.datePipe.transform(
                      wbgtmpDate.toString().replace(/\//g, ''),
                      'yyyy-MM-dd'
                    )
                    : this.datePipe.transform(new Date(), 'yyyy-MM-dd');
                  // labObj.time = wbgtmpDate
                  //   ? this.datePipe.transform(
                  //     wbgtmpDate.toString().replace(/\//g, ''),
                  //     'HH:mm:ss'
                  //   )
                  ...
                  //   )
                  //   : this.datePipe.transform(new Date(), 'HH:mm:ss');
                  labObj.treatingUnitCode = labObj.Trtoe;
                  labObj.treatingUnitDescription = labObj.TrtoeDescr;
                  labObj.defaultOrgCode = labObj.Orgfa;
                  labObj.defaultOrgDescription = labObj.OrgfaDescr;
                  labObj.isFastingRequired = labObj.Patnue;
                  labObj.note = labObj.Ergtx;
                  labObj.localization = labObj.Lslok;
                  labObj.duplicationKey = labObj.Cordtypid + labObj.id;
                });
              }

              if (obj.TORADSET?.results && obj.TORADSET?.results.length) {
                obj.TORADSET.results.forEach((radObj: any) => {
                  const wbgtmpDate = radObj.Wbgtmp
                    ? +radObj.Wbgtmp.replace(/[^0-9]/g, '')
                    : null;
                  radObj.einri = obj.einri;
                  radObj.falnr = obj.falnr;
                  radObj.lfdnr = obj.lfdnr;
                  radObj.Patnr = obj.Patnr;
                  radObj.catalogkey = radObj.Tarif;
                  radObj.clinOrdTypId = radObj.Cordtypid;
                  radObj.service = radObj.Talst;
                  radObj.serviceDescr = radObj.Servtxt;
                  radObj.id = radObj.service;
                  radObj.text = radObj.Servtxt;
                  radObj.isFavourite = radObj.Favourite;
                  radObj.icon = radObj.isFavourite ? 'star' : 'star_border';
                  radObj.isSelected = false;
                  radObj.type = 'RAD';
                  radObj.date = wbgtmpDate
                    ? this.datePipe.transform(
                      wbgtmpDate.toString().replace(/\//g, ''),
                      'yyyy-MM-dd'
                    )
                    : this.datePipe.transform(new Date(), 'yyyy-MM-dd');
                  // radObj.time = wbgtmpDate
                  //   ? this.datePipe.transform(
                  //     wbgtmpDate.toString().replace(/\//g, ''),
                  //     'HH:mm:ss'
                  //   )
                  ...
                  //   )
                  //   : this.datePipe.transform(new Date(), 'HH:mm:ss');
                  radObj.treatingUnitCode = radObj.Trtoe;
                  radObj.treatingUnitDescription = radObj.TrtoeDescr;
                  radObj.defaultOrgCode = radObj.Orgfa;
                  radObj.defaultOrgDescription = radObj.OrgfaDescr;
                  radObj.isFastingRequired = radObj.Patnue;
                  radObj.note = radObj.Ergtx;
                  radObj.localization = radObj.Lslok;
                  //radObj.duplicationKey = radObj.Cordtypid;
                  radObj.duplicationKey = radObj.Cordtypid + radObj.id;
                });
              }

              if (obj.TOPROCSET?.results && obj.TOPROCSET?.results.length) {
                obj.TOPROCSET.results.forEach((proObj: any) => {
                  const wbgtmpDate = proObj.Wbgtmp
                    ? +proObj.Wbgtmp.replace(/[^0-9]/g, '')
                    : null;
                  proObj.einri = obj.einri;
                  proObj.falnr = obj.falnr;
                  proObj.lfdnr = obj.lfdnr;
                  proObj.Patnr = obj.Patnr;
                  proObj.catalogkey = proObj.Tarif;
                  proObj.clinOrdTypId = proObj.Cordtypid;
                  proObj.service = proObj.Talst;
                  proObj.serviceDescr = proObj.Servtxt;
                  proObj.id = proObj.service;
                  proObj.text = proObj.Servtxt;
                  proObj.isFavourite = proObj.Favourite;
                  proObj.icon = proObj.isFavourite ? 'star' : 'star_border';
                  proObj.isSelected = false;
                  proObj.type = 'PRO';
                  proObj.date = wbgtmpDate
                    ? this.datePipe.transform(
                      wbgtmpDate.toString().replace(/\//g, ''),
                      'yyyy-MM-dd'
                    )
                    : this.datePipe.transform(new Date(), 'yyyy-MM-dd');
                  // proObj.time = wbgtmpDate
                  //   ? this.datePipe.transform(
                  //     wbgtmpDate.toString().replace(/\//g, ''),
                  //     'HH:mm:ss'
                  //   )
                  ...
                  //   )
                  //   : this.datePipe.transform(new Date(), 'HH:mm:ss');
                  proObj.treatingUnitCode = proObj.Trtoe;
                  proObj.treatingUnitDescription = proObj.TrtoeDescr;
                  proObj.defaultOrgCode = proObj.Orgfa;
                  proObj.defaultOrgDescription = proObj.OrgfaDescr;
                  proObj.isFastingRequired = proObj.Patnue;
                  proObj.note = proObj.Ergtx;
                  proObj.localization = proObj.Lslok;
                  //proObj.duplicationKey = proObj.Cordtypid;
                  proObj.duplicationKey = proObj.Cordtypid + proObj.id;
                });
              }

              if (obj.TOMEDICSET?.results && obj.TOMEDICSET?.results.length) {
                obj.TOMEDICSET.results.forEach((medObj: any) => {
                  medObj.einri = obj.einri;
                  medObj.falnr = obj.falnr;
                  medObj.lfdnr = obj.lfdnr;
                  medObj.Patnr = obj.Patnr;
                  medObj.text = medObj.RESULT_DRUG_NAME;
                  medObj.drugId = medObj.DRUGID;
                  medObj.formulaDesc = medObj.FORMATDESCR;
                  medObj.formulaId = medObj.PHFORMID;
                  medObj.dosageCount = medObj.QUAN;
                  medObj.dosageCountDesc = medObj.QUANUNIT;
                  medObj.unitDescription = medObj.QUANTUNITTXT;
                  medObj.routeId = medObj.APROUTEID;
                  medObj.routeDesc = medObj.ROUTEDESCR;
                  medObj.frequencyId = medObj.N1ZNR;
                  medObj.frequencyDesc = medObj.N1ZTXT;
                  medObj.durationCount = medObj.PDUR;
                  medObj.durationDesc = medObj.DURUNITTXT;
                  medObj.durationid = medObj.PDURU;
                  medObj.agentId = medObj.AGENTID;
                  medObj.prescriptionId = medObj.PRSCRID;
                  medObj.storn = medObj.STORN;
                  medObj.stoId = medObj.STOID;
                  medObj.updateMode = medObj.UPDMODE;
                  medObj.comment = medObj.DESCR;
                  medObj.isPrn = medObj.PRN === 'X';
                  medObj.prnText = medObj.PRNCOND;
                  medObj.Eordtyp = 'MED';
                  medObj.type = 'MED';
                  medObj.icon = medObj.Favourite ? 'star' : 'star_border';
                  medObj.duplicationKey = medObj.PRSCRID + medObj.DRUGID;
                  medObj.drugNo = medObj.DRUG;
                });
              }
            });
  ...
  onCreateFeeOrder() {
    ...
    // if (this.feeServiceOrderData[0].Orgid && this.feeServiceOrderData[0].Orgid !== '') {
    //   postObject['Orgid'] = this.feeServiceOrderData[0].Orgid;
    // }
