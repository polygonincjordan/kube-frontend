...
export class SidebarService {
  ...
  sidebar = new BehaviorSubject<ISidebar>(this.initialSidebar);
  ...
  setContainerClassnames(
    ...
    if (!selectedMenuHasSubItems) {
      if (
        currentClasses.includes('menu-default main-hidden sub-hidden') &&
        (clickIndex % 3 === 0 || clickIndex % 3 === 2)
      ) {
        clickIndex = 1;
      }
      // if (currentClasses.includes('menu-sub-hidden') && clickIndex % 3 === 1) {
      //   clickIndex = 0;
      // }
      if (
        currentClasses.includes('menu-hidden') &&
        (clickIndex % 3 === 1 || clickIndex % 3 === 2)
      ) {
        clickIndex = 0;
      }
    }
  ...
  clickOnMobileMenu(strCurrentClasses: string): void {
    const before = this.sidebar.getValue();
    const currentClasses = strCurrentClasses
      ? strCurrentClasses
          .split(' ')
