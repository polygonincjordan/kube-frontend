...
import { OrganizationUnitComponent } from '../e-order/organization-unit/organization-unit.component';
...
export class eOrderService {
  ...
  deleteOrderItemFromProfile(
    ...
    const hasLabRad =
      (postObject &&
        postObject.TOLABSET &&
        postObject.TOLABSET.length > 0) ||
      (postObject && postObject.TORADSET && postObject.TORADSET.length > 0);
    if (hasLabRad && sourceItems) {
      const protectedItems = findProtectedLabRadItems(sourceItems, true);
      if (protectedItems.length > 0) {
        this.showLabRadCancellationBlocked(protectedItems);
        if (errorCallback) {
          errorCallback();
              ...
              if (obj.TOLABSET?.results && obj.TOLABSET?.results.length) {
                obj.TOLABSET.results.forEach((labObj: any) => {
                  const wbgtmpDate = labObj.Wbgtmp
                    ? labObj.Wbgtmp.replace('/Date(', '').replace(')/', '')
                    : null;
