import { AfterContentChecked, ChangeDetectorRef, Component, ElementRef, HostListener, Input, OnInit, Renderer2, TemplateRef, ViewChild } from '@angular/core';
...
export class CreateDischargeOrderComponent implements OnInit , AfterContentChecked  {
  ...
  index: any;
  constructor(public ePrescriptionService: EPrescriptionService,public addministrationService: AddministrationService, private renderer: Renderer2, public route: ActivatedRoute,private modalService: BsModalService,public patientService: PatientService, private cdr: ChangeDetectorRef) { }
  ...
  someMethod(event?: any,term?: string) {
    if ((term !== undefined && term !== null && term.length >= 3)) {
  ...
  openPopup() {
    const inputElement = document.querySelector('.popupdata input');
    if (inputElement) {
