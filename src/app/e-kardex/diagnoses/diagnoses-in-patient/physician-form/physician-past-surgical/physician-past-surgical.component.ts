...
@Component({
  selector: 'app-physician-past-surgical',
  templateUrl: './physician-past-surgical.component.html',
  styleUrls: ['./physician-past-surgical.component.scss']
})
export class PhysicianPastSurgicalComponent implements OnInit {
  ...
  import() {
    let pastSurgFormitems = [];
   
      this.pastSurgFormitems.value.forEach(element => {
        if(element.isChecked) {
        pastSurgFormitems.push({
          Surgeryname:element.Surgeryname,
          Date: element.Date,
          Remarks: element.Remarks,
          Mode: element.Mode,
    ...
    this.importEvent.emit(pastSurgFormitems);
    this.resetupdateMedForm();
   this.modalRefForSurg.hide();
