...
import { PastSurgicalComponent } from './past-surgical/past-surgical.component';
import { StructureDocComponent } from './structure-doc/structure-doc.component';
...
export class PatientHistoryComponent implements OnInit {
  ...
  @ViewChild('diagnosisNotesKardexId') diagnosisNotesKardex: DiagnosisComponent;
  @ViewChild('physicianOrderKardexId') physicianOrderKardex: PhysicianOrderKardexComponent;
  @ViewChild('pastMedicalKardexId') pastMedicalKardex: PastMedicalComponent;
  @ViewChild('pastSurgicalKardexId') pastSurgicalKardex: PastSurgicalComponent;
  @ViewChild('structureDocKardexId') structureDocKardex: StructureDocComponent;
  ...
  constructor(private modalServiceForAllergy: BsModalService,
    private formBuilder: FormBuilder,public storageService: StorageService,private emergencyService:EmergencyService,private patientHistoryService:PatientHistoryService,private sanitizer: DomSanitizer,private hospitalistService: HospitalistService,private _dataServices: EEmrService) { 
  ...
  this.progressNotesKardex.openProgressNotesModal();
}
...
// phy order
openModalForPhysicianOrder() {
this.physicianOrderKardex.openModalForPhyOrder();
}
// past medical
...
openModalForFamilyHistory(){
  ...
  activateTabs(tabname){
    if (tabname == 'ProgressNotes') {
      ...
      this.openModalForProgressNotes();
      ...
      this.activeStructuredDoc = false;
      this.activeDiagnosis = false;
      this.openModalForPhysicianOrder();
      ...
      this.activePastSurgical = false;
      this.activeFamilyHistory = false;
      this.activeStructuredDoc = true;
      this.activeDiagnosis = false;
      this.openModalForStructureDoc();
  ...
  deleteRiskJson(item){
    this.riskJson = [];
    // var lfdnrValue = this.storageService.lfdnr.split('').slice(2);
    // var finallfdnrValue = lfdnrValue[0] + lfdnrValue[1] +lfdnrValue[2];
    this.riskJson = [{
      Patnr: this.storageService.patnr,
      Lfdnr: item.Lfdnr,
      Rsfnr: item.Rsfnr,
