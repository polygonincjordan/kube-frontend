  ...
  private _chartTooltip = {
    ...
    callbacks: {
      label: (tooltipItem, data)=> {
        const dataLabel = parseInt(data.datasets[tooltipItem.datasetIndex].data[tooltipItem.index]);
        const label = data.datasets[tooltipItem.datasetIndex].label;
        const labelDate = data.datasets[tooltipItem.datasetIndex].additionalData[tooltipItem.index];
        return `${dataLabel} ${label} - ${ this.convertTimestampToDate(labelDate)}`;
      }
    }
  };
