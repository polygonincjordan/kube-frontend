...
@Component({
  selector: 'premedications',
  templateUrl: './premedications.component.html',
  styleUrls: ['./premedications.component.scss']
})
export class PremedicationsComponent implements OnInit {
  ...
  @Input() set startdata(data: any) {
    if (data && data.length) {
      ...
      //     const calculate = this.calculateDataa(ele, data);
      //         this.drugArray.controls[index].patchValue({
      //           Quan: `${Math.ceil(calculate)}`
      //         });
  ...
  calculateDataa(ele: any, data: any): number {
    if (ele.value.Quanunit === "MG") {
      return data[0].Bsa * ele.value.Quan;
    } else if (ele.value.Quanunit === "each") {
      return data[0].Weight * ele.value.Quan;
    } else {
      return 0;
    }
  }
  ...
  ngOnInit() {
    ...
    this.modetailsFormSubscription = this.administrationForm.valueChanges.subscribe(() => {
      ...
      this.chemotherapyData.emit(this.drugArray);
      // this.generateDefaultForm();
    });
  ...
  premeicationssetData(resp) {
    if(this.isSelect){
        ...
        filterdata.forEach((item, index) => {
          if (notTouchedForms && notTouchedForms.length && notTouchedForms.length > index) {
            notTouchedForms[index].patchValue({
              ...
              Quan: `${Math.ceil(item.GenericDose)}` === '0.000' ? '0' : `${Math.ceil(item.GenericDose)}`,
              Quanunit: item.GenericUnit,
              ...
              Complex: item.Complex ? "X" : "",
              TOCOMPLEX: item.TOCOMPLEX = item.TOCOMPLEX != null ? item.TOCOMPLEX : [],
              TOEVENTDATA: item.TOEVENTDATA = item.TOEVENTDATA != null ? item.TOEVENTDATA : []
            });
      ...
      this.isSelect = true;
      this.CycleIdvalue =  resp.data['TOCHEMOPREMED'].results;
      const notTouchedForms = this.drugArray.controls;
      if (this.Protocalsubscription) { this.Protocalsubscription.unsubscribe(); }
      this.drugArray.reset();
        const filterdata =  resp.data['TOCHEMOPREMED'].results.filter(d => d.CycleId === resp.CycleId);
        filterdata.forEach((item, index) => {
          if (notTouchedForms && notTouchedForms.length && notTouchedForms.length > index) {
            notTouchedForms[index].patchValue({
              Agentid: item.Agentid,
  ...
  openMoDetailPanel(index: any, validData: any, isValidForm: boolean, IsMO: boolean) {
    if (isValidForm) {
      ...
      if ((this.drugArray.controls[index].get('TOEVENTDATA').value && !this.drugArray.controls[index].get('TOEVENTDATA').value.length)) {
        ...
        validData.Dosdef = validData.Dosdef !== null ? validData.Dosdef : "";
        validData.Moresp1 = this.addministrationService.medicationAdministrative.EmpResp;
        validData.TOCOMPLEX = validData.TOCOMPLEX = validData.TOCOMPLEX != null ? validData.TOCOMPLEX : [];
        ...
        delete validData.Lfdnr
        delete validData.Updmode;
        if (validData) {
