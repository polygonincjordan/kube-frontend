...
@Component({
  selector: 'chemo-discharge',
  templateUrl: './chemo-discharge.component.html',
  styleUrls: ['./chemo-discharge.component.scss']
})
export class ChemoDischargeComponent implements OnInit {
  ...
  cycleIdvalue:any = [];
  @Output() dichargeChemo = new EventEmitter();
  constructor(public ePrescriptionService: EPrescriptionService, public route: ActivatedRoute, public addministrationService: AddministrationService, public chemotherapyService: ChemotherapyService, private cdr: ChangeDetectorRef) { }
  ...
  ngOnInit() {
    ...
    for (let i = 1; i <= 4; i++) { this.drugArray.push(this.generateForm()); }
    this.isFormReady = true;
    this.chemotherapyService.ProtocalType.subscribe((resp) => {
      // this.chemotherapyService.protocalDetails(resp);
      this.isSelect = false;
      this.setmedical(resp);
    });
    this.chemotherapyService.mySubject.subscribe((res) =>{
      this.isSelect = true;
      this.setmedical(res);
    });
    this.modetailsFormSubscription = this.MedicationForm.valueChanges.subscribe((data) => {
         this.dichargeChemo.emit(this.drugArray);
     });
  }
      ...
      this.cycleIdvalue.filter(d => d.CycleId === resp).forEach((element, index) => {
        if (notTouchedForms && notTouchedForms.length && notTouchedForms.length > index) {
          notTouchedForms[index].patchValue({
            ...
            N1znr: element.N1znr,
            N1ztxt: element.N1znr,
            Result_Drug_Name: element.Descrlt,
            Quan: element.GenericDose,
            Quantunittxt: element.Gunittxt,
            Descr: element.Comments,
            Routedescr: element.Apdescr,
            Pdur: element.Duration,
            Pduru: element.DurationUnit,
            Aprouteid: element.Aprouteid,
            Prn: element.Prn,
            Prncond: element.Prncond,
            IsPatientMedication: true,
            AutoSelect: element.AutoSelect,
            Active: element.Active,
            Formatdescr: element.Phdescr,
            Phformid: element.Phformid,
            Quanunit:element.GenericUnit,
            Durunittxt:element.Dunittxt,
            Statustext:element.StartingOn,
            StartD: new Date(),
            StartT: this.parsePtTime(element.Uptim),
            EndD: this.sanitizeSAPDateFormat(element.Erdat, element.Ertim),
            EndT: this.parsePtTime(element.Ertim),
          });
        }
      });
    }else{
      this.isSelect = true;
      this.cycleIdvalue =  resp.data['TOCHEMODISCH'].results;
      const notTouchedForms = this.drugArray.controls;
      if (this.Protocalsubscription) { this.Protocalsubscription.unsubscribe(); }
      this.drugArray.reset();
      resp.data['TOCHEMODISCH'].results.filter(d => d.CycleId === resp.CycleId).forEach((element, index) => {
        if (notTouchedForms && notTouchedForms.length && notTouchedForms.length > index) {
          notTouchedForms[index].patchValue({
            Agentid: element.Agentid,
            Drugid: element.Drugid,
  ...
  generateForm() {
    return new FormGroup({
      ...
      StartD:new FormControl(new Date()),
