  ...
  onOpenTemplatePopup() {
    ...
    this.templateSubscription = this.templatePopup.onClose.subscribe(data => {
      ...
      for (let i = 0; i < data.length; i++) {
        if(data[i].Tmptype === "1"){
        this.ePrescriptionService.loadData(`e-prescription/OrderTemplateget?Einri=${this.ePrescriptionService.parameters.einri}&Falnr=${this.ePrescriptionService.parameters.falnr}&Tpgid=${data[i].Prscrid}&Ordtype=${'1'}`, false, false, false, false).subscribe((resp: any) => {
