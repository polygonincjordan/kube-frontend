...
@Component({
  selector: 'prior-complexorder',
  templateUrl: './prior-complexorder.component.html',
  styleUrls: ['./prior-complexorder.component.scss']
})
export class PriorComplexorderComponent implements OnInit {
      ...
      for (let i = 0; i < data.TOCOMPLEX.length; i++) {
        ...
        if (data.TOCOMPLEX[i].EndT === null) {
          if (formatDate(data.TOCOMPLEX[i].StartD, "HH:mm:ss") <= formatDate(new Date(), "HH:mm:ss")) {
            this.complexArray.controls[i].patchValue({
              StartD: new Date(data.TOCOMPLEX[i].StartD.setDate(data.TOCOMPLEX[i].StartD.getDate()))
            })
          }
