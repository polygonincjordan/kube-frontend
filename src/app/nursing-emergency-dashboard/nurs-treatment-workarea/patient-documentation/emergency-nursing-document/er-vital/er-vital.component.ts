...
import { VitalItem, CatalogItem, detailsVital, RangeTime } from '@services/e-kardex/interfaces/vitals';
...
@Component({
  selector: 'app-er-vital',
  templateUrl: './er-vital.component.html',
  styleUrls: ['./er-vital.component.scss']
})
export class ErVitalComponent implements OnInit {
  ...
  openModalForErVital(checkinitem) {
    ...
    // Use the vitalDefaultListResp already initialized in ngOnInit()
 ...
 EditVitalList() {
  ...
  CreateVitalList() {
    this.showMaintain = true;
    // Clear the form array first to prevent duplicates
    this.maintainVitalFormitems = this.maintainvitalform.get('maintainVitalFormitems') as FormArray;
    this.maintainVitalFormitems.clear();
