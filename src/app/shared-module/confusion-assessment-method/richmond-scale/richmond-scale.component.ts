...
export class RichmondScaleComponent implements OnInit {
  ...
  @ViewChild('scaleRichmondModal', { static: true }) scaleRichmondModal: TemplateRef<any>;
  ...
  getDocument(data?) {
    ...
    this.subscription = this.emergencyService.fetchRichmondDocument(this.dockeyValue).subscribe({
      next: (data: any) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.subscription = this.emergencyService.saveRichmondScaleDoc(payload).subscribe({
        next: (data: any) => {
          ...
          let formValue = {
            totalScore: this.richmondForm.value.TotalScore,
            description: this.richmondForm.value.ScoreDesc,
